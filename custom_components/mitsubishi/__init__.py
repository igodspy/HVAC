...
CONFIG_SCHEMA = vol.Schema(
    {vol.Optional(DOMAIN, default=[]): vol.All(cv.ensure_list, [MITSUBISHI_SCHEMA], _has_unique_names)},
...
def _create_device(hass, device, config_entry=None):
    """Create and register a Mitsubishi device handler."""
    name = device[CONF_NAME]
    temperature = device.get(TEMPERARURE_ENTITY, DEFAULT_TEMPERATURE_ENTITY)
    humidity = device.get(HUMIDITY_ENTITY, DEFAULT_HUMIDITY_ENTITY)
    remote_entity = device[REMOTE_ENTITY]
    try:
        api = MitsubishiHandler(
            hass,
            device=device,
            name=name,
        ...
        _LOGGER.error("Failed to set up Mitsubishi device %s: %s", name, ex)
        return False
    hass.data[DATA_MITSUBISHI][DEVICES][name] = MitsubishiDevice(
        api,
        config_entry,
    )
    return True
...
def _remote_area_id(hass, remote_entity):
    """Return the area assigned to a remote entity."""
...
def _register_ha_device(hass, device, config_entry_id=None):
    """Register a Mitsubishi AC in Home Assistant's device registry."""
    if config_entry_id is None:
        return None
    name = device[CONF_NAME]
    area_id = device.get(CONF_AREA_ID) or _remote_area_id(
        hass,
    ...
    device_kwargs["config_entry_id"] = config_entry_id
    device_entry = device_registry.async_get_or_create(**device_kwargs)
    if area_id and getattr(device_entry, "area_id", None) != area_id:
        try:
            ...
            _LOGGER.debug("Device registry does not support setting area_id")


def _remove_legacy_sensor_select_entities(hass, name):
    """Remove old sensor selector entities from the entity registry."""
    entity_registry = er.async_get(hass)
    for parameter in [TEMPERARURE_ENTITY, HUMIDITY_ENTITY]:
        unique_id = "_".join([name, parameter, "select"])
...
async def _async_import_yaml_device(hass, device):
    """Import a YAML AC into the config entry system."""
...
def _import_yaml_device(hass, device):
    """Import a YAML AC into the config entry system."""
    hass.add_job(_async_import_yaml_device, hass, dict(device))


async def _async_update_listener(hass, entry):
    """Reload a Mitsubishi AC when options are updated."""
...
def setup(hass, config):
    ...
    _prepare_integration(hass)
    for device in config.get(DOMAIN, []):
        _import_yaml_device(hass, device)
...
async def async_setup_entry(hass, entry):
    """Set up Mitsubishi Aircon from a config entry."""
    _prepare_integration(hass)
    device_config = {
        **entry.data,
        **entry.options,
    }
    if not _create_device(hass, device_config, entry):
        return False

    name = device_config[CONF_NAME]
    _remove_legacy_sensor_select_entities(hass, name)
    _register_ha_device(hass, device_config, entry.entry_id)
    _register_services(hass)
...
async def async_unload_entry(hass, entry):
    """Unload a Mitsubishi Aircon config entry."""
    unload_ok = await hass.config_entries.async_unload_platforms(entry, PLATFORMS)
    if unload_ok:
        hass.data[DATA_MITSUBISHI][DEVICES].pop(entry.data[CONF_NAME], None)
    return unload_ok
